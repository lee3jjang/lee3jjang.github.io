---
title: "Github 블로그 시작"
date: 2020-09-30 13:15:00 -0400
categories: 잡담
toc: true
toc_sticky: true
---

## 개요
Kotlin을 연습하는 공간입니다. Kotlin은 안드로이드 앱 개발에 필요한 언어입니다. 저는 갤럭시 유저이기 때문에 안드로이드 앱을 만들면 좋겠다고 생각해서 Kotlin을 배우기로 했습니다.

## 컨텐츠
* Kotlin 문서화 연습
* Markdown 연습
* 안드로이드 앱 개발 연습
* Kotlin의 문법들을 연습
* Java로 구현 ESG 프로그램 Kotlin으로 이식

## 최종목표
1. ESG 패키지 개발
2. 안드로이드 앱 개발


## 참고자료
* Kotlin 설치 및 개발환경 설정은 여기 [블로그](https://webnautes.tistory.com/1148)를 참조하였습니다.
* 블로그 꾸미기는 여기 [블로그](https://hahafamilia.github.io/howto/jekyll-github-mistakes-blog/) 및 여기 [블로그](https://dreamgonfly.github.io/blog/jekyll-remote-theme/)를 참고했습니다.
* 댓글 기능 추가는 여기 [블로그](https://jamesu.dev/posts/2020/01/03/adding-disqus-comment-service-to-jekyll/)를 참고했습니다.